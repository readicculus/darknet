...
eval_data* get_eval_data(char *datacfg){
    eval_data* ed = (eval_data*)xmalloc(sizeof(eval_data));
...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        */
        ...
        calc_map_for_each =  500; //fmax(calc_map_for_each, 500);
        ...
        if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches)) {
            ...
            iter_map = iteration;
            mean_average_precision = validate_detector_map(datacfg, cfgfile, weightfile, 0.25, 0.5, 0, net.letter_box, &net_map, &cur_eval_data);// &net_combined);
...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, eval_data* cur_eval_data)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.txt");
    ...
    for (i = 0; i < classes; ++i) {
        ...
        printf("class_id = %d, name = %s, ap = %2.2f%%   \t (TP = %d, FP = %d, FN = %d) \n",
